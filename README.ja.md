...
現在はソースコードからビルドして使う方式です。各ユーザーのMac上でad-hoc署名されたアプリを生成するため、Apple Developer登録は必要ありません。
...
- 実際の天気、権限変更、スリープ復帰、ログイン起動、ディスプレイ再接続は各Macでの手動確認が必要です。
...
プロジェクトのライセンスはまだ決定していません。一般公開前に`LICENSE`を追加します。
